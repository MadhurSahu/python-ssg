...
def generate_pages_recursive(basepath, dir_path_content, template_path, dest_path):
    files = os.listdir(dir_path_content)
    for file in files:
        file_path = os.path.join(dir_path_content, file)
        file_dst_path = os.path.join(dest_path, file.replace(".md", ".html"))
        if os.path.isfile(file_path):
            generate_page(basepath, file_path, template_path, file_dst_path)
        else:
            generate_pages_recursive(basepath, file_path, template_path, file_dst_path)
    return
